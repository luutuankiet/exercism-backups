...
export function buildBirthdaySign(age) {
  const referrer = age >=50 ? 'mature' : 'young'
  return `Happy Birthday! What a ${referrer} fellow you are.`;
}
...
export function costOf(sign, currency) {
  const basePrice = 20;
  const baseLetterPrice = 2;
  const cost = (baseLetterPrice * sign.length) + basePrice;
